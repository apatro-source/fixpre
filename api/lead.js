...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      await sql`insert into leads (id, email, name, staff, chefs, venues, note, org_id)
        values (${genId()}, ${email}, ${name}, ${staff}, ${chefs}, ${venues}, ${note}, ${orgId})`;
    ...
    if (req.method === "GET") {
      ...
      const rows = await sql`select id, email, name, staff, chefs, venues, note, org_id, created_at
